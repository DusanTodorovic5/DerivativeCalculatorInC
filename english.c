...
int main() {
	...
	char zaizvod = 'A';
	while (1) {
		printf("Enter number of command to execute it :  ");
		scanf("%d", &izbor);
		printf("\n\n");
		if (izbor == 0) break;
		switch (izbor) {
		...
		case 2:if (stablo != NULL) { printf("Prefix : "); printPrefix(stablo); 	printf("\n\n"); }
			  else printf("Enter the binary tree first !!\n\n");
			break;
		...
		case 6: if (stablo != NULL) {
			printf("Enter the variable by which you differentiate  : ");
...
void PrintDrvo(tnode* stablo) {
	...
	//pushtn(&stek, p);
	int level = 0;
	printf("\n#####    Horizontal view, above is the left son, and below is the right son      #####\n\n");
	do {
		while (p != NULL) {
...
Tablica* unosVrednosti(Tablica* tab) {
	...
	char c; double t;
	for (int i = 0; i < n; i++) {
		printf("Which variable do you want to enter? :  ");
		scanf(" %c", &c);
...
void stampajTekst() {
	...
	printf("#                                                       #\n");
	printf("# 1 -> Create tree from infix                           #\n");
	printf("# 2 -> Print prefix expression                          #\n");
	printf("# 3 -> Print tree                                       #\n");
	printf("# 4 -> Enter variable values                            ​​#\n");
	...
	printf("#########################################################\n");
	printf("#                                                       #\n");
	printf("# Logarithm A with base B is written l (A, B)           #\n");
	printf("# Exponent of A is written e (A)                        #\n");
	printf("# The absolute value of A is written a (A)              #\n");
	printf("# !! The program cannot differentiate X ^ X !!          #\n");
	printf("#                                                       #\n");
	printf("# If no variable value is entered, they                 #\n");
	printf("# get the value of the ordinal number in the alphabet   #\n");
	printf("#                                                       #\n");
	printf("#########################################################\n\n");
}
